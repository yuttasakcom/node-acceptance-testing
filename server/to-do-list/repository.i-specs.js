...
describe("toDoListRepository", () => {
  ...
  context("when creating and getting a list", () => {
    ...
    beforeEach(() => {
      return toDoListRepository
        .createEmptyList(userId, "Default")
  ...
  context("when adding items to a todo list", () => {
    context("when creating and adding one item to a list", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          ...
          .then(l => (savedList = l))
          .then(() =>
            toDoListRepository.addItem(userId, savedList._id, itemName)
          )
          .then(l => (savedList = l));
      });
    ...
    context("when adding multiple items to a list", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          ...
          .then(l => (savedList = l))
          .then(() =>
            toDoListRepository.addItem(userId, savedList._id, item1Name)
          )
          .then(() =>
            toDoListRepository.addItem(userId, savedList._id, item2Name)
          )
          .then(() =>
            toDoListRepository.addItem(userId, savedList._id, item3Name)
          )
          .then(l => (savedList = l));
      });
    ...
    context("when the list does not exist", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          .addItem(userId, listId, itemName)
          .then(l => (savedList = l));
      });
    ...
    context("when the user does not own the list", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          .createEmptyList(userId, "My Test List")
          .then(l => (createdList = l))
          .then(() =>
            toDoListRepository.addItem(otherUserId, createdList._id, "New Item")
          )
          .catch(err => (error = err))
  ...
  context("when checking an item as complete", () => {
    context("when the item exists", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          ...
          .then(() =>
            toDoListRepository.checkItem(item1Id, expectedCompletedAt)
          )
          .then(r => (response = r))
          .then(() => toDoListRepository.getList(userId))
          .then(l => {
    ...
    context("when the item DOES NOT exist", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          .checkItem(itemId, new Date().toLocaleString())
          .catch(err => (error = err));
      });
    ...
    context("when the item exists", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          ...
          .then(() =>
            toDoListRepository.checkItem(item1Id, expectedCompletedAt1)
          )
          .then(() =>
            toDoListRepository.checkItem(item2Id, expectedCompletedAt2)
          )
    ...
    context("when the item exists", () => {
      ...
      beforeEach(() => {
        return toDoListRepository
          ...
          .then(() =>
            toDoListRepository
              .removeItem(userId, listId, item1Id)
              .then(r => (response = r))
              .then(() => toDoListRepository.getList(userId))
  ...
  context("when removing a list", () => {
    ...
    beforeEach(() => {
      return toDoListRepository
        .createEmptyList(userId, "List To Delete")
        .then(l => (listId = l._id))
